...
class DataLoader:
    ...
    def load_folder(self, folder_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        logger.info(f"Folder processing complete. Processed: {processed_files}, Skipped: {skipped_files}")
        return result
    ...
    def _load_json(self, filepath: Path) -> Dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            return {table_name: self._create_table_structure(data, table_name, filepath)}
    ...
    def _load_csv(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {table_name: self._enhance_table_metadata(table_data, table_name, filepath)}
    ...
    def _load_excel(self, filepath: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_name in excel_file.sheet_names:
                ...
                table_data = self._dataframe_to_table_structure(df)
                result[table_name] = self._enhance_table_metadata(table_data, table_name, filepath)
    ...
    def _dataframe_to_table_structure(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col_name in df.columns:
            ...
            records = self._convert_to_serializable(column_data[:self.max_records_per_column])
    ...
    def _enhance_table_metadata(self, table_data: Dict[str, Any], table_name: str, filepath: Path) -> Dict[str, Any]:
        ...
        country_info = self.country_extractor.process_filename_country(filepath.name)
        
        # Add enhanced metadata
        table_data['metadata'].update({
            'country': country_info.get('standardized_name'),
    ...
    def _create_table_structure(self, data: Any, table_name: str, filepath: Path) -> Dict[str, Any]:
        ...
        if isinstance(data, dict):
            ...
            for col_name, col_data in data.items():
                if isinstance(col_data, list):
                    records = self._convert_to_serializable(col_data[:self.max_records_per_column])
                    columns[col_name] = {'records': records}
                else:
                    columns[col_name] = {'records': [self._convert_to_serializable([col_data])[0]]}
                ...
                for record in data[:self.max_records_per_column]:
                    for key, value in record.items():
                        if key not in columns:
                            columns[key] = {'records': []}
                        columns[key]['records'].append(self._convert_to_serializable([value])[0])
            else:
                # List of values
                columns = {'data': {'records': self._convert_to_serializable(data[:self.max_records_per_column])}}
        else:
            # Single value
    ...
    def _convert_to_serializable(self, values: List[Any]) -> List[Any]:
        ...
        for val in values:
            ...
            elif pd.isna(val) if pd else (val is None or (isinstance(val, float) and np.isnan(val))):
                result.append(None)
            else:
