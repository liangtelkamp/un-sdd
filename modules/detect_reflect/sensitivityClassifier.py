...
import os
import torch
from .prompt_register import is_pii, PII_ENTITIES_LIST, is_sensitive_pii, prompt_non_pii, prompt_non_pii_table, prompt_reflect_non_pii, prompt_non_pii_table_no_isp
...
class SensitivityClassifier:
    ...
    def __init__(self, model_name):
        ...
        self.model, self.tokenizer, self.client, self.model_type = self.model_instance.get_model_components()
    ...
    def classify_pii(self, column_name, sample_values):
        ...
        """
        if not any(char.isalpha() or char.isdigit() for value in sample_values for char in str(value)):
            return 'None'
    ...
    def classify_sensitive_pii(self, column_name, context, pii_entity, max_new_tokens=128):
        """
        Classify if a column contains PII based on its name and sample values.
    ...
    def classify_sensitive_non_pii(self, column_name, table_context, isp, max_new_tokens=256):
        """
        Classify if a column contains PII based on its name and sample values.
        """
        ...
        try:
            ...
            if 'gemma' in self.model_name:
                ...
                prediction = prediction[response_index + len('<|CHATBOT_TOKEN|>'):].strip()
                explanation = prediction
