        ...
        stats.update(
            {
                "gpu_memory_allocated_gb": round(
                    torch.cuda.memory_allocated() / (1024 * 1024 * 1024), 3
                ),
                "gpu_memory_reserved_gb": round(
                    torch.cuda.memory_reserved() / (1024 * 1024 * 1024), 3
                ),
...
def update_peak_memory(
    current_stats: Dict[str, Any], peak_stats: Dict[str, Any]
) -> None:
    """Update peak memory usage statistics."""
    peak_stats["peak_ram_used_gb"] = max(
        peak_stats.get("peak_ram_used_gb", 0), current_stats["ram_used_gb"]
    )
    peak_stats["peak_ram_percent"] = max(
        peak_stats.get("peak_ram_percent", 0), current_stats["ram_percent"]
    )
...
def main():
    ...
    logging.info(
        f"Memory usage after model loading: {json.dumps(post_model_memory, indent=2)}"
    )
    ...
    for key, value in tqdm(csv_data.items(), desc="Processing tables"):
        ...
        value = reflect_pii(
            value,
            generator,
            detect_key="pii_detection_/home/ltelkamp/detecting-sensitive-data/results/unsloth/Qwen3-8B-ft",
            reflect_key="pii_detection_qwen3_8B_ft_reflection_gpt_4o_mini",
        )
        csv_data[key] = value
    ...
    average_time_per_table = (
        sum(table_processing_times) / len(table_processing_times)
        if table_processing_times
        else 0
    )
    ...
    logging.info(f"Average time per table: {average_time_per_table:.2f} seconds")
    logging.info(
        f"Peak RAM usage: {peak_memory_stats.get('peak_ram_used_gb', 0):.3f} GB ({peak_memory_stats.get('peak_ram_percent', 0):.1f}%)"
    )

    if torch.cuda.is_available():
        logging.info(
            f"Peak GPU memory allocated: {peak_memory_stats.get('peak_gpu_memory_allocated_gb', 0):.3f} GB"
        )
        logging.info(
            f"Peak GPU memory reserved: {peak_memory_stats.get('peak_gpu_memory_reserved_gb', 0):.3f} GB"
        )
